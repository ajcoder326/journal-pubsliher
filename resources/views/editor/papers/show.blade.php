            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    @if($paper->document_path)
                        <a href="{{ asset('storage/' . $paper->document_path) }}" class="btn btn-outline-primary"
                            target="_blank">
                    ...
                    @forelse($paper->reviews as $review)
                        <div class="border-bottom pb-3 mb-3">
                            <div class="d-flex justify-content-between">
                                ...
                                <span
                                    class="badge bg-{{ $review->recommendation == 'accept' ? 'success' : ($review->recommendation == 'reject' ? 'danger' : 'warning') }}">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <p><strong>Current Status:</strong>
                        <span
                            class="badge bg-{{ $paper->status == 'pending' ? 'warning' : ($paper->status == 'published' ? 'success' : 'info') }}">
                    ...
                    <form action="{{ route('editor.papers.update', $paper) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="status" class="form-select">
                                ...
                                <option value="in_review" {{ $paper->status == 'in_review' ? 'selected' : '' }}>In Review
                                </option>
                                <option value="correction_needed" {{ $paper->status == 'correction_needed' ? 'selected' : '' }}>Correction Needed</option>
                                <option value="approved" {{ $paper->status == 'approved' ? 'selected' : '' }}>Review Done -
                                    Approved</option>
                                <option value="rejected" {{ $paper->status == 'rejected' ? 'selected' : '' }}>Review Done -
                                    Rejected</option>
                            </select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select id="review_template" class="form-select">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="editor_feedback" id="editor_feedback" rows="6"
                                class="form-control">{{ $paper->editor_feedback }}</textarea>
                        </div>

                        <button type="submit" class="btn btn-primary w-100">Update Status</button>

                        <script>
                            document.getElementById('review_template').addEventListener('change', function () {
                                const content = this.value;
                                if (content) {
                                    document.getElementById('editor_feedback').value = content;
                                }
                            });
                        </script>
                    </form>
